...
$table = "course_subject_link as c,subject_master as s,course_table as ct,course_level as cl";
...
if ($action == "delete") {
    $inputId = $_REQUEST['courseid'];
    $courseLevelId = $_REQUEST['courselevelid'];
    $query = "DELETE from `course_subject_link` where `Course_Id`='" . $inputId . "' and Course_Level_Id = '" . $courseLevelId . "'";
...
$where_field = " WHERE 1 and c.Course_Level_Id=cl.Course_Level_Id and c.Course_Id=ct.CourseId and c.Subject_Id=s.Subject_Id" . $where_field;
                ...
                <form name="f1" method="post" action="<?= $listPage ?>?action=search">
                    <tr>
                        <td width="25%"><select name="search_for">
                                ...
                                <option value="s.Subject_Name" <?php if ($search_for == "s.Subject_Name") { ?> selected="selected"<?php } ?>>Subject Name</option>
                            </select></td>
    ...
    <tr>
        <td>
            <table cellpadding="0" cellspacing="0" width="100%" align="left">
                ...
                <td class="endcap"></td>
                <td style="padding-left:10px;" align="right"><a href="<?= $addPage ?>?edit=2">Add New Link</a></td>
                <td align="left"></td>
            </table>
        </td>
    </tr>    
        ...
        <td class="mainarea" align="Left">		
            <table cellpadding="5" cellspacing="5" align="center" width="98%" border="1" style="border-collapse:collapse;" bordercolor="#cacaca" >
                ...
                <tr align="center" class="colhead">
                    ...
                    <td width="24%">Mandatory Subject</td>
...
while ($f_arr = f($q_arr)) {
    ...
    ?>
                    ...
                    echo $fcl['Course_Level_Name'];
                    ?></td>
                        <td>
    <?php
    $cl = $f_arr['Course_Id'];
    $fcl = mysql_fetch_array(mysql_query("select * from course_table where CourseId='$cl'"));
    echo $fcl['Course_Name'];
    ?></td>
                        <td><?php
                        $cl1 = $f_arr['Subject_Id'];
                        $fcl = mysql_fetch_array(mysql_query("select * from subject_master where Subject_Id='$cl1'"));
                        echo $fcl['Subject_Name'];
    ?><?php echo $f_arr['']; ?></td>
                        <td align="center"><!--<a href="item_view?id=<?php echo $f_arr['id']; ?>"><image src="images/view.png" width="20" height="20" ></a>&nbsp;&nbsp;<image src="images/pa.jpg" width="3" height="20">-->&nbsp;&nbsp;<a href="<?= $addPage ?>?id=<?php echo $f_arr['id']; ?>&edit=1"><image src="images/edit.png" width="20" height="20"></a>&nbsp;&nbsp;
                            <?php
                            $itemQuery = mysql_fetch_array(mysql_query("SELECT COUNT(*) as `count` FROM `stock` WHERE `item_id`='" . $f_arr['id'] . "'"));

                            if ($itemQuery['count'] > 0) {
                                
                            } else {
                                ?> 
                                <image src="images/pa.jpg" width="3" height="20">&nbsp;&nbsp;<a href="<?php echo $_SERVER['PHP_SELF'] ?>?action=delete&courseid=<?php echo $cl; ?>&courselevelid=<?php echo $f_arr['Course_Level_Id'] ?>"><image src="images/delete.png" width="20" height="20"></a>
                            <?php } ?> 
