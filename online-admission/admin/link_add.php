...
if($action=='Submit'){
	...
	if($chkNumb>0 && $chkid['id']!=$id ){
		$print="<font color='#FF0000'>Link already exists</font>";
			...
			if($edit==1){
				mysql_query("update ".$table." set `Course_Id`='$CourseId',`Course_Level_Id`='$Course_Level_Id',`Subject_Id`='$Subject_Id' where id='$id'") or die(mysql_error());
				header('location:'.$listPage);
				$print="<font color='#FF0000'>Value successfully Inserted.</font>";
					...
					$sql=mysql_query("INSERT INTO ".$table." (`Course_Id`,`Course_Level_Id`,`Subject_Id`) "
                                                . "VALUES ('$CourseId','$Course_Level_Id','$Subject_Id')") or die(mysql_error());
