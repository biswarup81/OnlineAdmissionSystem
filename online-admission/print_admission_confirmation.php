...
<TR>
	...
	<td width="10%"><img alt="Passport size photo" src="<?php echo './pg/ulasset/'.$admin->getAppAttachmentDetails($details->user_id,2);?>" width="112" height="108"></td>
</TR>
...
?>
	...
	<tr><td colspan="4" align="center" >PAYMENT DETAILS | <?php echo "Student Reference: ".$details->Application_No;?></td></tr>
        <tr><td colspan="2" align="center" class="sTD">APPLICATION FEE</td><td colspan="2" align='center'>ADMISSION FEE</td></tr>
       <tr>
         <?php if($details->Application_Fee == 'challan'){?>
            <td class="sTD" colspan="2" >Payment Mode:- Offline Payment Done<br/>Enclose:- Application Challan</td>
           <?php }
             else if($details->Application_Fee == 'paytm'){?>
                 <?php $sql = "SELECT*FROM application_registration_payment WHERE Application_No ='$details->Application_No' ";
                      $result = mysql_query($sql) or die('connection error'.mysql_error());
                      $paymentObj = mysql_fetch_object($result) ;?>
               <td class="sTD" colspan="2">Payment Mode:- Online Payment Done<br/>TAXN ID: <?php echo $paymentObj->Unique_No; ?>
                <br/>GATEWAY: <?php echo $paymentObj->Gateway; ?><br/>Payment Date: <?php echo $paymentObj->Payment_date; ?></td>
           <?php } else{?>
                  <td class="sTD" colspan="2" >No Payment Record Found</td>
             <?php }?>
           
          <?php if($details->Admission_Fee == 'challan'){?>
            <td  colspan="2">Payment Mode:- Offline Payment Done<br/>Enclose:- Admission Challan</td>
           <?php }
             else if($details->Admission_Fee == 'paytm'){?>
                 <?php $sql1 = "SELECT*FROM admission_payments  WHERE Application_No ='$details->Application_No' ";
                      $result1 = mysql_query($sql1) or die('connection error'.mysql_error());
                      $paymentObj1 = mysql_fetch_object($result1) ;?>
               <td colspan="2">Payment Mode:- Online Payment Done<br/>TAXN ID: <?php echo $paymentObj1->Unique_No; ?>
                <br/>GATEWAY: <?php echo $paymentObj1->Gateway; ?><br/>Payment Date: <?php echo $paymentObj1->Payment_date; ?></td>
           <?php } else{?>
                  <td colspan="2" >No Payment Record Found</td>
             <?php }?>
 
       
       </tr>
          <tr><td colspan="4" align="center" >OFFLINE PAYMENT DETAILS (FOR VERIFICATION)</td></tr>
         <tr>
          <td class="sTD" colspan="2"><label>AC Name : KANDRA RADHAKANTA KUNDU MAHAVIDYALAYA</label></td>
	  <td class="sTD" colspan="2"><label>AC Number:<br/>SBI - 11782685061<br/>Axis - 919010011021249 </label></td>
	</tr>
    <tr>
